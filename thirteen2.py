...
class PlayerGrid(GridLayout):
	...
	def __init__(self, player, **kwargs):
		...
		self.pos_hint = self.set_position(self.player.name)
		for card in self.player.hand: self.add_widget(CardButton(card))
...
class GameScreen(Screen):
	...
	def display_grids(self, game):
		...
		self.ids.layout.add_widget(FieldGrid(game.field))
...
class Field():
	def __init__(self, game):
		self.game = game	
		self.current = {"play": None, "player": self.who_goes_first(self.game.players)}

	def who_goes_first(self, players): #2
		values = []
	...
	def new_play(self, play):	#called by Player()'s play() method
		if play == "pass":
			self.game.next_turn()
...
class Player():
	...
	def play(self): 
		...
		if selected:
			...
			self.hand = hand_copy
			self.game.field.new_play(Play(selected))
		else:
			self.game.field.new_play("pass")
