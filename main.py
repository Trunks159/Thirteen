...
class CardImage(Image):
    ...
    def __init__(self, card, **kwargs):
        ...
        self.card = card
        self.states = {"normal": "Pictures/Cards/" + self.card.face +
                       self.card.suit + ".png", "down": "Pictures/Cards/back.png"}
        self.state = "normal"
...
class HowManyPlayers(Screen):  # NOT IMPLEMENTED FULLY
    ...
    # button info that changes from btn to label depending on if an option was selected
    shape_shifter_o = ObjectProperty(None)

    # removes button or label and replaces it
    def on_shape_shifter_o(self, instance, value):
...
class Human(ToggleButton):  # NOT IMPLEMENTED FULLY
    def on_state(self, instance, value):
        if value == "down":
            ...
            hmp.shape_shifter_o = Button(
                text=hmp.ids.shape_shifter.text, size_hint=hmp.ids.shape_shifter.size_hint, pos_hint=hmp.ids.shape_shifter.pos_hint)

        elif value == "normal":
            hmp.humans = ""
            hmp.shape_shifter_o = Label(
                text=hmp.ids.shape_shifter.text, size_hint=hmp.ids.shape_shifter.size_hint, pos_hint=hmp.ids.shape_shifter.pos_hint)
    ...
    def on_current_play(self, instance, value):
        ...
        if value.cards:
            for card in value.cards:
                self.add_widget(CardImage(card))
        else:
...
class CardButton(ToggleButton):
    def __init__(self, card, **kwargs):
        ...
        self.card = card
        self.background_normal = "Pictures/Cards/" + \
            self.card.face + self.card.suit + ".png"
...
class PlayerGrid(GridLayout):
    ...
    def __init__(self, game, player, **kwargs):
        ...
        self.game = game
        self.pos_hint = {
            "y": .05} if self.player.name == "Player 0" else {"top": .95}
...
class OrderButton(Button):
    def __init__(self, game, player, **kwargs):
        ...
        self.game = game
        self.pos_hint = {
            "y": .05 + .2} if self.player.name == "Player 0" else {"top": .95 - .2}
        self.color = [1, .5, .5,
                      1] if self.player.name == "Player 0" else [.5, .5, 1, 1]
...
class CombosGrid(GridLayout):
    ...
    def __init__(self, game, **kwargs):
        ...
        for combo in self.combos:
            self.add_widget(
                Label(text=combo.title(), color=self.default_color))
...
class Field():
    ...
    def play(self, play, players):  # called by Player()'s play() method
        ...
        if current_play:
            print("New play = ", play.get_combo(),
                  ", Current Play = ", current_play.get_combo())
...
class Player():
    ...
    def play(self):
        ...
        while i < len(hand_copy):
            ...
            i += 1
        if selected:
            print("Old hand len: ", len(self.hand),
                  "new_hand length", len(hand_copy))
            play = Play(selected)
            if play.get_combo():
                self.hand = hand_copy
                return play
    ...
    def order_hand(self):
        ...
        # converts all the keys to an ordered list
...
class Play():
    ...
    def get_combo(self):
        if self.cards == "pass":
            ...
            return self.isChain(self.cards)
